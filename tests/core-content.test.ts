...
describe('strips that move and strips the learner fills', () => {
  type Draft<Payload> = { blockId: string; kind: string; typeVersion: number; required: boolean; payload: Payload };
  const sequence: Draft<{ parts: number; alt: string; frames: { filled: number; caption?: string }[]; frameMs?: number; loop?: boolean }> = {
    blockId: 'fraction-meaning:s1:sequence:v1', kind: 'math.fraction_sequence', typeVersion: 1, required: true,
    payload: {
      parts: 4, alt: '4등분한 막대가 한 칸씩 세 칸까지 채워지는 장면',
      frames: [{ filled: 0, caption: '빈 막대' }, { filled: 2 }, { filled: 3, caption: '$\\frac{3}{4}$' }],
    },
  };
  const builder: Draft<{ parts: number; target: number; start?: number; prompt: string; promptAlt?: string }> = {
    blockId: 'fraction-meaning:s1:builder:v1', kind: 'math.fraction_builder', typeVersion: 1, required: true,
    payload: { parts: 6, target: 4, prompt: '6칸 중 4칸을 채워 보세요.' },
  };
  const withBlock = (block: unknown) => {
    const record = structuredClone(seedClasses[0]);
    record.sections[0].contentBlocks.push(structuredClone(block) as never);
    return record;
  };

  it('registers both formats so a published class can rely on them', () => {
    expect(supportedBlockTypes).toContainEqual({ kind: 'math.fraction_sequence', typeVersion: 1 });
    expect(supportedBlockTypes).toContainEqual({ kind: 'math.fraction_builder', typeVersion: 1 });
    expect(() => validateClass(withBlock(sequence))).not.toThrow();
    expect(() => validateClass(withBlock(builder))).not.toThrow();
  });

  it('keeps every frame inside the strip and asks for at least a pair of them', () => {
    const overflowing = structuredClone(sequence);
    overflowing.payload.frames[1].filled = 5;
    expect(() => validateClass(withBlock(overflowing))).toThrow(/filled must not exceed parts/);
    const single = structuredClone(sequence);
    single.payload.frames = [{ filled: 1 }];
    expect(() => validateClass(withBlock(single))).toThrow(/Invalid payload/);
  });

  it('lets a frame caption carry math while the animation is named in plain words', () => {
    const named = structuredClone(sequence);
    named.payload.alt = '$\\frac{3}{4}$이 채워지는 장면';
    expect(() => validateClass(withBlock(named))).toThrow(/math markup/);
  });

  it('bounds the frame interval so a sequence cannot be published faster than it can be read', () => {
    const quick = structuredClone(sequence);
    quick.payload.frameMs = 50;
    expect(() => validateClass(withBlock(quick))).toThrow(/Invalid payload/);
    quick.payload.frameMs = 1200;
    expect(() => validateClass(withBlock(quick))).not.toThrow();
  });

  it('keeps a tapped strip coarse enough to hit and gives it something to do', () => {
    const crowded = structuredClone(builder);
    crowded.payload.parts = 20;
    expect(() => validateClass(withBlock(crowded))).toThrow(/Invalid payload/);
    const unreachable = structuredClone(builder);
    unreachable.payload.target = 7;
    expect(() => validateClass(withBlock(unreachable))).toThrow(/target must not exceed parts/);
    const finished = structuredClone(builder);
    finished.payload.start = 4;
    expect(() => validateClass(withBlock(finished))).toThrow(/target must differ/);
  });

  it('requires a plain promptAlt when the instruction carries math', () => {
    const asked = structuredClone(builder);
    asked.payload.prompt = '$\\frac{4}{6}$만큼 놓아 보세요.';
    expect(() => validateClass(withBlock(asked))).toThrow(/promptAlt/);
    asked.payload.promptAlt = '6분의 4만큼 놓아 보세요.';
    expect(() => validateClass(withBlock(asked))).not.toThrow();
  });

  it('keeps a manipulative away from anything that is marked', () => {
    const record = structuredClone(seedClasses[0]);
    record.problems[0].promptContent.push(structuredClone(builder) as never);
    expect(() => validateClass(record)).toThrow(/An interactive block is not allowed inside a problem/);
    const hinted = structuredClone(seedClasses[0]);
    hinted.problems[0].hints.push(structuredClone(builder) as never);
    expect(() => validateClass(hinted)).toThrow(/An interactive block is not allowed inside a problem/);
    expect(() => termContentBlockSchema.parse(structuredClone(builder))).toThrow(/interactive block/);
    // A definition may still show the movement it describes.
    expect(() => termContentBlockSchema.parse(structuredClone(sequence))).not.toThrow();
  });

  it('shows the frames a question illustrates without letting the learner act inside it', () => {
    const record = structuredClone(seedClasses[0]);
    record.problems[0].promptContent.push(structuredClone(sequence) as never);
    expect(() => validateClass(record)).not.toThrow();
  });
});
