...
export function DiagnosticPanel({ diagnostic, offering, dispatch, busy, onBack }: {
  ...
  return <section className="diagnostic-panel">
    ...
    {!diagnostic ? offering ? <div className="lesson-sheet"><h2>{offering.title} · {offering.total}문제</h2><p>약 {offering.estimatedMinutes}분이 걸려요. 점수를 매기기보다 지금 필요한 수업을 찾는 데 사용해요. 저장한 답은 변경할 수 없고, 결과는 마지막에 함께 확인해요.</p><p>나중에 돌아와도 저장한 문제 다음부터 이어갈 수 있어요. 진단 없이 클래스에서 바로 시작해도 괜찮아요.</p><button className="button primary" disabled={busy} onClick={() => void start()}>시작점 확인하기</button></div> : <p>시작점 확인을 준비하고 있어요. 클래스에서 학습을 시작할 수 있어요.</p>
