...
import type { ActionResponse, AssignmentView, AttemptView, LessonDocument, ContentBlock, EnrollmentView, Goal, LearningAction, LearningState, PublicLesson, PublicProblem, PublicConcept } from '@/shared/api';
...
const assignmentKindLabel: Record<AssignmentView['policy']['kind'], string> = { review: '나의 학습과 연결된 복습', homework: '숙제', exam: '시험' };
/** A due date is a deadline and is said first; a review only has a recommended moment. */
const assignmentTiming = (item: Pick<AssignmentView, 'recommendedAt' | 'opensAt' | 'dueAt'>) =>
  item.dueAt ? `${formatDate(item.dueAt)}까지` : item.opensAt ? `${formatDate(item.opensAt)}부터` : `${formatDate(item.recommendedAt)} 권장`;
...
export function LearningWorkspace() {
  ...
  async function refresh(restoreAfterLogin = false) {
    ...
    try {
      ...
      setCatalog(publicCatalog.lessons); setTaughtConcepts(publicCatalog.concepts); setState(nextState);
  ...
  function assignmentRow(item: AssignmentView) {
    ...
    return <button key={item.recipientId} className="assignment-row" onClick={() => openAssignment(item)}><span className="assignment-icon"><Icon name={item.status === 'submitted' ? 'check' : 'pencil'} size={23} /></span><span className="assignment-info"><strong>{item.title}</strong><small>{item.items.length}문제 · {assignmentTiming(item)}</small></span><span className={`assignment-status ${item.status}`}>{item.status === 'submitted' ? '제출 완료' : savedCount ? `${savedCount}/${item.items.length} 저장` : '풀어보기'}</span><Icon name="chevron" size={18} /></button>;
  }
  ...
  function renderHome() {
    return <>
      ...
      {state && <section className="personalization-card" aria-label="나에게 맞는 학습">
        ...
        <div className="readiness-list">{state.plan.readiness.map(concept => <span key={concept.key} className={`readiness ${concept.readiness}`}><strong>{concept.label}</strong> · {concept.readiness === 'ready' ? '다음 개념 준비' : concept.readiness === 'needs-practice' ? '한 번 더 연습' : '아직 확인 전'}{concept.source === 'diagnostic' ? ' (진단 기준)' : ''}</span>)}</div>
  ...
  function renderHistory() {
    ...
    return <><div className="page-heading"><div className="eyebrow">EVERY SMALL STEP COUNTS</div><h1>조금씩 쌓이는 나의 이해.</h1><p>빠르기보다, 어제보다 조금 더 이해하는 것. 여기까지 온 걸음을 확인해요.</p></div>{!state ? <EmptyState title="첫 걸음을 기록해 보세요" text="내 학습을 시작하면 수업 진도와 개념별 학습 기록이 여기에 모여요." actionLabel="내 학습 시작하기" onAction={() => setModal('login')} /> : <><div className="history-stats"><div><small>완료한 수업</small><strong>{completedCount}<span>개</span></strong></div><div><small>제출한 과제</small><strong>{state.assignments.filter((item) => item.status === 'submitted').length}<span>개</span></strong></div><div><small>풀어본 문제</small><strong>{state.enrollments.reduce((sum, item) => sum + new Set(item.attempts.map((attempt) => attempt.problemVersionId)).size, 0) + state.assignments.reduce((sum, item) => sum + item.items.filter((entry) => entry.attempt).length, 0)}<span>개</span></strong></div></div><section className="dashboard-section"><div className="section-heading"><h2>개념별 학습 상태</h2><span className="muted small">풀이와 복습 기록을 기준으로 반영해요</span></div><div className="concept-list">{state.concepts.map((concept) => <div key={concept.key}><span className={`concept-dot ${concept.state}`} /><strong>{concept.label}</strong><span className={`concept-state ${concept.state}`}>{labels[concept.state]}</span></div>)}</div></section><section className="dashboard-section"><div className="section-heading"><h2>추천이 바뀐 기록</h2></div><p className="muted small">학습과 설정을 저장할 때 달라진 추천을 최근 10개까지 보여줘요.</p><div className="recommendation-history">{state.recommendationHistory.map(entry => <article key={entry.id}><small>{formatDate(entry.createdAt)}</small>{entry.recommendations.map(item => <div key={item.lessonKey}><strong>{lessons.find(c => c.lessonKey === item.lessonKey)?.title ?? '이전 수업'}</strong><p>{item.reason}</p><small>하루 계획 {item.suggestedMinutes}분</small></div>)}</article>)}</div></section><section className="dashboard-section"><div className="section-heading"><h2>학습 중인 수업</h2></div><div className="class-grid">{lessons.filter((item) => state.enrollments.some((enrollment) => enrollment.lessonKey === item.lessonKey)).map((item) => <LessonCard key={item.lessonKey} item={item} index={lessons.indexOf(item)} enrollment={state.enrollments.find((entry) => entry.lessonKey === item.lessonKey)} onOpen={() => void openLesson(item.lessonKey)} />)}</div>{!state.enrollments.length && <EmptyState title="아직 시작한 수업이 없어요" text="첫 수업에서 새로운 이해를 만나보세요." actionLabel="수업 둘러보기" onAction={() => navigate('lessons')} />}</section></>}</>;
  }
