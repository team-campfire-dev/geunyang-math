...
type Page = 'home' | 'classes' | 'practice' | 'history' | 'lesson' | 'assignment' | 'diagnostic';
...
export function LearningWorkspace() {
  ...
  const activeNav = page === 'lesson' ? 'classes' : page === 'assignment' ? 'practice' : page === 'diagnostic' ? 'home' : page;
  ...
  function renderHome() {
    return <>
      ...
      {state && <section className="personalization-card" aria-label="나에게 맞는 학습">
        ...
        <div className="recommendation-choice"><button className="text-button" disabled={busy} onClick={() => navigate('classes')}>다른 수업 직접 고르기 →</button>{state.plan.preferredClassKey && <button className="text-button" disabled={busy} onClick={() => { void dispatch({ action: 'recommendation.choose', classKey: null }).catch(() => {}); }}>자동 추천으로 돌아가기</button>}</div>
