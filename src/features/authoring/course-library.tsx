...
import { lessonMatches } from '@/shared/authoring-checks';
...
export function CourseLibrary({ workspace, busy, courseKey, onCourse, onOpen, onAction, onDirty }: {
  workspace: AuthoringWorkspace; busy: boolean; courseKey: string | null; onCourse: (key: string | null) => void;
  onDirty?: (dirty: boolean) => void; onOpen: (draft: DraftSummary) => void; onAction: (action: AuthoringAction) => Promise<boolean>;
}) {
  ...
  const found = (lesson: typeof workspace.lessons[number]) => lessonMatches(lesson, workspace.drafts, query);
  const dirty = !!(newLesson?.title || newLesson?.conceptKeys.length || conceptName || (courseForm && (courseForm.creating ? courseForm.title || courseForm.summary : courseForm.title !== course?.title || courseForm.summary !== course?.summary)));
  useUnsavedForm(dirty, onDirty);
  const changeCourse = (key: string | null) => { if (!discardChanges(dirty)) return; onCourse(key); setQuery(''); setNewLesson(null); setCourseForm(null); setConceptName(''); };
  ...
  const addConcept = async () => {
    if (busy || !conceptName.trim()) return;
    const key = newKey('concept');
    if (!(await onAction({ action: 'concept.create', key, label: conceptName.trim() }))) return;
    setNewLesson(value => value ? { ...value, conceptKeys: [...value.conceptKeys, key] } : value);
    setConceptName('');
  };
  ...
  return <>
    ...
    <div className="section-heading studio-heading">
      ...
      <div className="editor-actions">{course ? <>
        {admin && <button className="button secondary" disabled={busy} onClick={() => setCourseForm(current => current ?? { ...course, creating: false })}>코스 정보</button>}
        <button className="button primary" disabled={busy} onClick={() => setNewLesson(current => current ?? { key: newKey('lesson'), title: '', conceptKeys: [] })}><Icon name="plus" size={16} />수업 추가</button>
      </> : admin && <button className="button primary" disabled={busy} onClick={() => setCourseForm(current => current ?? { key: newKey('course'), title: '', summary: '', creating: true })}><Icon name="plus" size={16} />코스 만들기</button>}</div>
    </div>
    ...
    {courseForm && <form className="editor-panel studio-form" onSubmit={async (event) => {
      ...
      <div className="editor-actions"><button className="button primary" disabled={busy || !courseForm.title.trim() || (courseForm.creating && !lessonKeyPattern.test(courseForm.key))}>저장</button><button type="button" className="text-button" disabled={busy} onClick={() => { if (discardChanges(dirty)) setCourseForm(null); }}>취소</button></div>
    </form>}

    {newLesson && course && <form className="editor-panel studio-form" onSubmit={async (event) => {
      event.preventDefault(); if (conceptName.trim()) return; await onAction({ action: 'lesson.create', courseKey: course.key, lessonKey: newLesson.key, title: newLesson.title.trim(), conceptKeys: newLesson.conceptKeys });
      ...
      <ConceptPicker concepts={workspace.concepts.filter((item) => item.assessable)} chosen={newLesson.conceptKeys} label="수업에서 다루는 개념" onChange={(conceptKeys) => setNewLesson({ ...newLesson, conceptKeys })} />
      {admin && <details className="studio-new-concept"><summary>목록에 없는 개념 추가</summary><p className="editor-note">개념은 수업을 이루는 작은 수학 단위예요. 예: 분수, 분자, 분모, 미지수, 꼭짓점. 수업 제목 대신 개념 이름을 쓰고, 기존 개념이 있으면 함께 사용해 주세요.</p>
        <label className="editor-field"><span className="editor-label">새 개념 이름</span><input maxLength={191} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void addConcept(); } }} value={conceptName} onChange={(e) => setConceptName(e.target.value)} /></label>
        <button type="button" className="button secondary" disabled={busy || !conceptName.trim()} onClick={() => void addConcept()}>개념 추가하고 선택</button>
      </details>}
      <div className="editor-actions"><button className="button primary" disabled={busy || !newLesson.title.trim() || !!conceptName.trim() || !lessonKeyPattern.test(newLesson.key)}>수업 만들기</button><button type="button" className="text-button" disabled={busy} onClick={() => { if (discardChanges(dirty)) { setNewLesson(null); setConceptName(''); } }}>취소</button></div>
      <p className="editor-note">수업이 다루는 개념을 골라 주세요. 같은 개념의 뜻풀이와 관련 수업을 함께 찾을 수 있어요. 지금 건너뛰어도 되고, 발행 전에 고르면 됩니다.{conceptName.trim() && ' 입력 중인 개념은 먼저 추가하거나 지워 주세요.'}</p>
    </form>}
    ...
    {course && <section className="dashboard-section"><div className="section-heading"><h3>수업 순서 <span className="count-label">{lessons.length}</span></h3>
      ...
      <div className="studio-lessons">{lessons.map((lesson, index) => {
        ...
        return <article className="studio-lesson" key={lesson.lessonKey}><span className="studio-order">{String(index + 1).padStart(2, '0')}</span>
          <div className="assignment-info"><h3>{lesson.title}</h3><small>{lesson.latestVersionId ? `${versionLabel(lesson.latestVersionId)} 발행됨` : '아직 발행하지 않았어요'}{editor ? ` · ${lesson.lessonKey}` : ''}</small>
            ...
            {lesson.latestVersionId && <button className="button secondary" disabled={busy} onClick={() => { if (discardChanges(dirty)) void onAction({ action: 'lesson.read', versionId: lesson.latestVersionId! }); }}>발행판 보기</button>}
      ...
      {!lessons.some((lesson) => found(lesson)) && <p className="empty-inline">{query ? '찾은 수업이 없어요.' : '첫 수업을 추가해 보세요.'}</p>}
    </section>}
