...
export const misconceptions: MisconceptionRecord[] = [
  ...
  // 지수법칙
  { key: 'multiply-exponents-on-product', label: '같은 밑을 곱할 때 지수를 곱하기',
    note: '$a^2\\times a^3$은 $a^6$이 아니라 $a^5$이에요. 곱하면 지수는 더해요.' },
  { key: 'add-exponents-on-power', label: '거듭제곱의 거듭제곱에서 지수를 더하기',
    note: '$(a^2)^3$은 $a^5$이 아니라 $a^6$이에요. 거듭제곱의 거듭제곱은 지수를 곱해요.' },
  { key: 'add-terms-as-powers', label: '더하기에도 지수법칙을 쓰기',
    note: '$a^2+a^3$은 하나로 합쳐지지 않아요. 지수법칙은 곱하고 나눌 때의 규칙이에요.' },
  // 명제와 집합
  { key: 'vague-collection-as-set', label: '기준이 분명하지 않은 모임을 집합으로 보기',
    note: '「키가 큰 사람」은 어디부터인지 정할 수 없어 집합이 아니에요.' },
  { key: 'false-is-not-proposition', label: '거짓인 문장은 명제가 아니라고 보기',
    note: '참인지 거짓인지 가릴 수 있으면 명제예요. 거짓이어도 명제예요.' },
  { key: 'contrapositive-confused', label: '대우를 역이나 이와 섞기',
    note: '대우는 앞뒤를 바꾸고 둘 다 부정한 것이에요. 하나만 하면 역이거나 이예요.' },
  { key: 'converse-always-true', label: '명제가 참이면 역도 참이라고 보기',
    note: '참인 명제와 늘 함께 참인 것은 대우 하나뿐이에요.' },
  { key: 'example-not-counterexample', label: '반례 대신 성립하는 예를 고르기',
    note: '반례는 가정을 만족하면서 결론은 만족하지 않는 것이에요.' },
  // 이차부등식
  { key: 'inequality-region-flipped', label: '이차부등식의 해를 바깥과 안쪽 반대로 잡기',
    note: '아래로 볼록한 식이 $0$보다 크면 두 근의 바깥, 작으면 두 근의 사이예요.' },
  { key: 'intersection-as-between', label: '겹치지 않는 두 구간의 사이를 해로 잡기',
    note: '두 조건을 모두 만족하는 곳이 없으면 해가 없어요. 사이가 해가 되지는 않아요.' },
  { key: 'one-condition-only', label: '한 조건만 보고 답하기',
    note: '「이면서」로 이어진 조건은 둘이 겹치는 곳만 해예요.' },
  { key: 'discriminant-sign-confused', label: '판별식의 부호를 반대로 읽기',
    note: '판별식이 음수면 만나지 않고, 양수면 두 점에서 만나요.' },
  // 다항식의 나눗셈
  { key: 'remainder-sign-flipped', label: '나머지정리에서 부호를 반대로 넣기',
    note: '$x-5$로 나눈 나머지는 $f(5)$예요. 넣는 값은 나누는 식을 $0$으로 만드는 수예요.' },
  { key: 'quotient-remainder-swapped', label: '몫과 나머지의 자리를 바꾸기',
    note: '$A=BQ+R$이에요. 나누는 식에 곱해지는 것이 몫이에요.' },
  { key: 'divide-until-zero', label: '나머지가 0이 될 때까지 나눠야 한다고 보기',
    note: '나머지의 차수가 나누는 식보다 낮아지면 거기서 끝이에요.' },
  // 복소수
  { key: 'conjugate-flips-real', label: '켤레복소수에서 실수부의 부호까지 바꾸기',
    note: '켤레는 허수부의 부호만 바꿔요. 실수부는 그대로예요.' },
  { key: 'i-power-cycle', label: '허수 단위의 거듭제곱 주기를 잘못 세기',
    note: '$i,\\ -1,\\ -i,\\ 1$이 네 개마다 되풀이돼요.' },
  { key: 'sum-sign-in-quadratic', label: '근의 합을 부호 그대로 계수에 쓰기',
    note: '두 근의 합이 $4$면 $x$의 계수는 $-4$예요.' },
  { key: 'sum-product-swapped', label: '근의 합과 곱의 자리를 바꾸기',
    note: '합은 $x$의 계수 자리로, 곱은 상수항 자리로 가요.' },
  // 함수
  { key: 'composition-order-flipped', label: '합성함수의 순서를 반대로 보기',
    note: '$g(f(x))$는 $f$를 먼저 지나고 그 값을 $g$에 넣는 것이에요.' },
  { key: 'inverse-as-axis-reflection', label: '역함수를 축에 대한 대칭으로 보기',
    note: '역함수의 그래프는 직선 $y=x$에 대해 뒤집은 모양이에요.' },
  { key: 'asymptote-as-crossing', label: '점근선을 만나는 선으로 보기',
    note: '점근선은 한없이 다가가되 닿지 않는 선이에요.' },
  { key: 'domain-condition-swapped', label: '정의역 조건을 다른 함수의 것과 섞기',
    note: '무리함수는 근호 안이 $0$ 이상, 유리함수는 분모가 $0$이 아닐 것이에요.' },
  { key: 'parallel-perpendicular-swapped', label: '평행 조건과 수직 조건을 바꾸기',
    note: '기울기가 같으면 평행, 기울기의 곱이 $-1$이면 수직이에요.' },
  // 검산
  { key: 'recheck-by-repeating', label: '같은 계산을 다시 하는 것을 검산으로 보기',
    note: '같은 길로 두 번 가면 같은 실수를 두 번 해요. 역연산으로 되돌려 봐요.' },
  // 도형 — 합동과 닮음
