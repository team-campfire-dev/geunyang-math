...
등록 전 전체 참조와 문서 형식을 검사한다. 알 수 없는 필수 블록, 중복 ID, 없는 개념, 바뀐 기존 판본, 기존 문제 ID의 다른 내용, 클래스와 진단에 겹치는 문제 ID는 거부한다. 용어에서는 없는 `termKey` 참조, 없는 `skillKey`, 판본마다 달라지는 용어의 개념, 본문에 없는 표층 문자열, 자신이 평가하는 개념을 문항 안에서 설명하는 주석을 거부한다. 쓰기는 한 DB 트랜잭션으로 처리하며 `--dry-run`은 쓰지 않는다.
