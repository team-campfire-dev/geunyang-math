...
2026-09-22 확인 기준 최근 릴리스는 `0062330` (#129)다 — 같은 날 공개 `/api/version`이 이 commit을, `/api/health`가 `ready`를 돌려주었다. 그 앞의 `e4ca467`(#128)까지도 같은 방식으로 main이 검증하고 배포했다. 아래 「릴리스별 검증 기록」은 **모든 릴리스의 목록이 아니라 배운 것이 있는 릴리스의 기록**이다 — 저장 구조를 옮긴 묶음, migration이 권한에 막힌 날, 발행된 판본을 고쳐 배포가 멈춘 날처럼. 나머지 릴리스의 검증은 PR과 GitHub Actions에 그대로 남아 있고, 지금 무엇이 떠 있는지는 언제나 `/api/version`이 답한다.
...
| 앱 VM | Cooo와 같은 VM, 사설 `10.0.0.130`, aarch64. 공인 IP는 저장소에 적지 않는다 — [아래](#저장소를-공개로-돌린-뒤) |
| SSH 사용자 | `ubuntu` |
...
**한 커밋은 한 번만 검증한다**(2026-09-21). 전에는 `push`에 브랜치 제한이 없어 브랜치 커밋 하나가 `push`와 `pull_request`로 두 번 돌았고, 두 실행의 ref가 달라(`refs/heads/…` vs `refs/pull/…/merge`) 서로 취소하지도 않았다. `pull_request`는 **그 커밋을 main에 합친 결과**를 검사하므로 브랜치 push의 실행이 더 증명하는 것이 없다. 이제 `push`는 `main`에서만 돈다. 이 이유는 청구와 무관하다 — 같은 커밋에 대한 두 번째 판정은 확신이 아니라 소음이다.

**릴리스 포장 검사는 매 실행마다 돈다.** 한때 포장 면(`Dockerfile`·`prisma/`·`package-lock.json`과 migrator가 읽는 소스)을 건드리는 PR에서만 돌게 했으나, 그 조건의 근거는 청구되는 분을 아끼는 것 하나였다. 저장소를 공개로 돌려 분이 무료가 된 지금 그 절약은 아무것도 사지 않고 실패를 뒤로 미루기만 하므로 되돌렸다.

측정해 둔다 — 비공개였던 30일 동안 실행 294회·약 1,640분이었고, 한 실행은 290초였다(테스트 84초, 포장 검사 62초, MySQL 기동 32초, 두 번의 Next 빌드 각 31초). 2026-09-21에 저장소를 공개로 돌리면서 청구는 사라졌다. 실행 시간 자체는 그대로이므로, 결과를 기다리는 시간이 문제가 되면 모바일 export를 `main`에서만 돌리기(약 31초)와 테스트 파일 병렬 실행(DB 테스트끼리 충돌하지 않게 나눠야 한다)이 남아 있다.
...
2026-09-21에 저장소를 공개로 바꿔 Actions 분 청구를 없앴다. 바꾼 뒤 이력 전체를 훑어 개인 키·토큰·자격 증명이 커밋된 적이 없음을 확인했다. 이름이 그럴듯한 파일은 값이 비어 있는 `.env.example`과 공개 CA 인증서뿐이다.

**공개된 것은 origin 주소다.** 공개 도메인은 Cloudflare 뒤에 있어 DNS는 대리 주소만 답하지만, 이 문서가 앱 VM의 공인 IP를 적고 있었다. 그 IP에 `Host` 헤더를 붙여 부르면 origin이 직접 200을 돌려준다 — Cloudflare를 건너뛸 수 있다는 뜻이다. 문서에서는 지웠으나 **공개된 이력에는 남아 있어 문서만으로는 닫히지 않는다.** 실질적인 처리는 origin 방화벽에서 443을 Cloudflare 대역으로만 여는 것이고, 아직 하지 않았다.

포크에서 온 PR은 `verify`만 돌릴 수 있다. 이 job은 비밀값을 하나도 쓰지 않고, 배포 job은 `main`의 push·수동 실행으로 제한되어 포크가 닿지 않는다. 최초 기여자의 워크플로는 승인을 받아야 돈다(`first_time_contributors`).

공개 저장소에서 무료가 되었지만 아직 켜지 않은 것 — secret scanning과 push protection, Dependabot 보안 업데이트. `GITHUB_TOKEN`의 기본 권한도 여전히 write다. 이 워크플로는 `permissions: contents: read`로 스스로 좁히지만, 나중에 더해질 워크플로는 기본값을 물려받는다.
