...
2026-09-17 작성 · 갱신: 2026-09-23 · 코드 기준: `4308a83` (#134). 그 묶음은 빌드도 Capacitor 설정도 건드리지 않았고, 공용 계약에 `plan.onTheWay` 하나가 늘었다 — 정적 export는 그대로 통과한다.
...
- 수업 콘텐츠와 개인 과제는 빌드 후에도 추가된다. 공용 학습 화면은 고정된 `/` 진입점의 상태·query로 대상 ID를 선택한다 — 2026-09-22부터 **서 있는 자리가 전부 query에 적힌다**(`?page=`·`?lesson=`(+`&step=`)·`?set=`). 경로가 아니라 query인 것이 이 경계다: 정적 번들에는 `index.html` 하나뿐이라 만들지 않은 경로는 그 안에서 죽은 링크다. 주소가 가리키는 것은 열기 전에 카탈로그와 학습자의 것인지로 검사한다. 미리 생성할 수 없는 `[classId]` 경로나 요청에 의존하는 Server Component·Server Action을 학습 흐름의 필수 경로로 두지 않는다. `useSearchParams`가 필요해지면 `Suspense` 경계에 둔다.
- 웹은 같은 origin의 `/api`를 호출한다. 모바일은 빌드 시 지정한 HTTPS API origin을 사용한다. 미디어 주소 역시 이 경계를 통해 해석한다. 컴포넌트마다 API 주소를 조합하지 않는다.
- 그래프·도형 등 새 블록도 공용 렌더러에 추가한다. 현재 그림·장면·놓기 활동은 `core.scene@1`로 통합했다. 이전 분수 전용 sequence/builder 블록은 제거했다. 움직임은 재생·멈춤·앞뒤 이동 버튼과 함께 제공하고 동작 줄이기 설정에 양보하며, 조작은 누르기와 키보드를 기본으로 두고 드래그는 마우스에서만 더한다. 조작 결과는 서버로 보내지 않으므로 채점·진도와 무관하다. `kind`와 `typeVersion`으로 지원 여부를 판단하며, 미지원 필수 블록을 빈 화면이나 학습 완료로 처리하지 않는다. 대체 설명을 표시해도 해당 상호작용을 수행한 것으로 채점하지 않는다. 새 블록 발행은 구버전 앱의 렌더러 지원 범위를 고려해야 한다.
...
공유 `ConceptExplorer`와 `POST /api/v1/definitions` 계약을 웹·정적 번들에서 함께 사용한다. 화면 폭에 따라 옆 패널 또는 모달 시트를 사용하며 모바일 시트는 확장·배경 비활성화·포커스 복귀를 제공한다. 브라우저 history로 탐색 한 단계를 되돌리며, Capacitor/Android 네이티브 뒤로 가기 연동은 아직 검증하지 않았다. 학습 화면도 2026-09-22부터 화면마다 history 항목을 남기므로 네이티브 뒤로 가기가 갈 곳은 생겼지만, 실기 확인은 여전히 하지 않았다. 관련 수업은 `/?lesson=...`를 새 탭에서 열고 현재 수업을 보존한다. 정적 빌드 성공은 실제 기기에서 새 창·인증이 동작한다는 보장이 아니다. 자세한 계약과 검증 범위는 [개념 탐색](concept-exploration-design.md)을 따른다.
