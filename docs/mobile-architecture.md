...
- 클래스 콘텐츠와 개인 과제는 빌드 후에도 추가된다. 공용 학습 화면은 고정된 `/` 진입점의 상태·query로 대상 ID를 선택한다. 미리 생성할 수 없는 `[classId]` 경로나 요청에 의존하는 Server Component·Server Action을 학습 흐름의 필수 경로로 두지 않는다. `useSearchParams`가 필요해지면 `Suspense` 경계에 둔다.
...
클래스와 진단 정의·개념은 서버 DB에서 읽고 추천·채점도 서버에서 수행한다. 정적 번들에 기본 강의나 진단 문항·정답을 하드코딩하지 않는다. 기존 블록 형식의 새 DB 콘텐츠는 같은 공개 API로 전달하므로 앱 재빌드 없이 읽을 수 있다. 새 블록의 검증·렌더러는 별도 코드 변경이 필요하고 구버전 앱의 지원 여부를 확인해야 한다. 현재 인증이 준비되지 않은 native 앱에서 이 API를 사용할 수 있다는 보장은 아니다.
