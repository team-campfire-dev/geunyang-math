...
이 문서의 스키마 예시와 초기화 전제는 당시 변경 작업의 기록이다. 이후 기본 수업 재작성(#47), 뜻풀이 맥락·개념 탐색(#48)은 [현재 구현 현황](implementation-status.md)과 [개념 탐색 설계](concept-exploration-design.md)를 따른다. 현재 스키마에는 `ConceptDefinition.usageNote`가 있으며 summary는 선택 요약이다.

기준일: 2026-09-16 · 상태: **완료(2026-09-17).** 아래 「확정된 결정」의 권장안대로 A 개명, B 코스·정체 표, C 개념, D 문제집, E 과제, F 진단을 한 PR씩(#37·#40·#41·#42·#43·#44) 반영해 운영에 배포했다. 남은 것은 선택인 G(squash)뿐이다. 이 문서는 그 결정과 순서의 기록으로 남긴다.
