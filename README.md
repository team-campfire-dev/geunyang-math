...
현재 [운영 웹](https://geunyang-math.team-campfire.dev/)에서 **Google 로그인 → 선택형 진단 → 맞춤 추천 → 수업 학습 → 개인 복습 과제 → 제출·기록 복원**을 제공합니다. 2026-09-21 코드 기준은 답을 쓰는 숫자 키패드까지입니다. MySQL 통합 검사와 화면 렌더 검사를 포함한 599개 테스트와 웹·모바일 빌드를 통과했습니다. 실제 배포 버전은 아래 배포 기록에서 별도로 확인합니다. 기능별 구현·후속 범위는 [구현 현황](docs/implementation-status.md), 릴리스별 근거는 [배포 기록](docs/deployment.md#릴리스별-검증-기록)에서 확인합니다.

## 현재 구현

- 분수에서 시작해 소수 · 비와 비율 · 정수와 유리수를 지나 문자와 식 · 일차방정식 · 식의 계산 · 일차부등식 · 연립방정식 · 일차함수로 이어지는 **코스 10개, 수업 30개** — 중학교 2학년 과정까지
...
- 시작점 확인 문항 95개가 카탈로그의 개념 48개를 개념마다 둘씩 묻습니다. 은행이 두 배가 되어도 한 사람이 받는 문항 수는 늘지 않습니다
...
- 답을 쓸 때 **숫자 키패드**가 분수 선과 음수 부호를 내주고, 쓴 답을 수식으로 그려 보여 줍니다
...
진단·첫 풀이·선행개념에 따른 규칙 기반 추천과 시간에 맞춘 새 복습 과제를 제공합니다. 수업·진단 문항·개념·용어는 DB에서 읽으며, `/authoring` 화면이나 [콘텐츠 관리 명령](docs/content-management.md)으로 새 판본을 발행합니다. 화면에 아직 없는 것은 일반 이미지·판본 비교이고, AI 호출은 없습니다. 콘텐츠는 중학교 2학년의 대수 줄기까지이며 도형·확률과 전문가 검수는 아직입니다.
