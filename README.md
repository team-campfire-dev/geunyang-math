...
현재 [운영 웹](https://geunyang-math.team-campfire.dev/)에서 **Google 로그인 → 선택형 진단 → 맞춤 추천 → 수업 학습 → 개인 복습 과제 → 제출·기록 복원**을 제공합니다. 2026-09-23 코드 기준은 무엇을 배우러 왔는지 먼저 묻는 첫 걸음, 한 번에 하나만 권하는 홈, 「내 과정」으로 열리는 수업 목록, 그리고 코스로 답하는 「지금 어디쯤인가」까지입니다(#134). MySQL 통합 검사와 화면 렌더 검사를 포함한 772개 테스트와 웹·모바일 빌드를 통과했습니다. 실제 배포 버전은 아래 배포 기록에서 별도로 확인합니다. 기능별 구현·후속 범위는 [구현 현황](docs/implementation-status.md), 릴리스별 근거는 [배포 기록](docs/deployment.md#릴리스별-검증-기록)에서 확인합니다.

## 현재 구현

- 기초 과정(분수 · 소수 · 비와 비율)에서 시작해 중학교 과정과 고등학교 1학년으로 이어지는 **코스 34개, 수업 106개** — **중학교 과정 전체와 고등학교 1학년을 온전히 채웠습니다.** 다음은 고등학교 2학년 과정
- 학교 줄기 옆에 **NCS 수리영역 코스 4개, 수업 21개** — 기초연산·기초통계·도표분석·도표작성 네 영역에 일률 · 이자 · 환율 · 확률 · 그래프 읽기 · 조건 걸어 고르기까지. **도표작성은 학습자가 직접 옮겨 그립니다**
- 코스의 순서는 번들이 적습니다. 아무것도 고르지 않은 학습자는 그 순서만으로 추천받기 때문입니다
- **처음 들어온 학습자에게 무엇을 배우러 왔는지 먼저 묻습니다** — 어디쯤에서 왔는지(줄기·학년) → 그 안의 과정 → 하루 몇 분, 세 걸음입니다. 서른여덟 개를 한 목록에 늘어놓는 대신 학년을 먼저 물어 고를 것을 네댓 개로 줄이고, 「잘 모르겠어요」를 고르면 시작점 확인이 그 답을 대신 찾습니다. 계정마다 한 번만 묻습니다
- **배우려는 과정을 바꾸면 시작점 확인이 따라갑니다** — 이미 답한 문항은 다시 묻지 않고, 새 과정에 필요해진 것만 이어서 물어요. 끝난 확인도 그래서 몇 가지 더 열릴 수 있고, 이미 확인한 것으로 충분하면 더 묻지 않아요
- **「지금 어디쯤인가」를 개념이 아니라 코스로 답합니다** — 내 학습에 배우려는 과정까지 가는 길이 한 줄로 놓입니다. 지나온 코스는 세고, 남은 코스는 차례로 이름을 부르며 개념 몇 개 중 몇 개가 정해졌는지 말합니다. 학습 기록의 개념별 상태도 코스별로 묶이고, 코스를 열면 그 개념들이 그대로 있습니다
- **수업 화면은 카탈로그 전체가 아니라 「내 과정」으로 열립니다** — 배우려는 과정과 지금 배우는 코스, 그리고 추천이 가리키는 코스입니다. 「전체」에서는 코스 38개의 이름과 진행만 서고 수업 카드는 열었을 때만 그려집니다. 이름으로 찾으면 범위와 상관없이 카탈로그 전체에서 찾습니다
- **내 학습은 한 번에 한 가지만 권합니다** — 밀린 복습이 있으면 그것, 하다 만 시작점 확인이 있으면 그것, 기록이 없으면 시작점 확인, 그다음이 추천 수업입니다. 나머지는 「이 추천은 이렇게 정했어요」로 접힙니다
- 앞의 답을 따라 다음 문제를 고르는 시작점 확인 — 한 문제의 답이 그 위나 아래 개념까지 함께 정합니다. **보기에서 고르는 문항은 보기를 함께 내보냅니다**
- 배우려는 과정을 고르면 거기까지 가는 데 필요한 것만 확인하고, 추천도 그쪽을 향합니다
- 시작점 확인 문항 327개가 카탈로그의 개념 164개를 개념마다 둘씩 묻습니다. 은행이 두 배가 되어도 한 사람이 받는 문항 수는 늘지 않습니다
...
- 답을 쓸 때 **숫자 키패드**가 분수 선과 음수 부호를 내주고, 쓴 답을 수식으로 그려 보여 줍니다. 휴대폰 키보드를 함께 띄우지 않고 **대신합니다** — 키보드로 쓰고 싶으면 한 번 눌러 바꿉니다. 키보드가 엔터로 끝나듯 키패드도 자기 **보내는 키**로 끝나고, 한 번 누르면 저장됩니다
- **틀린 답을 읽습니다** — 부호, 뒤집힌 분수, 열 배와 백 배의 자릿값, 하나 차이의 셈. 문항이 자기 오답에 **이름표**를 달아 두었으면(지금 서른일곱 코스의 233문항) 그 착각의 설명이 그대로 갑니다. 다만 시험처럼 결과를 미루는 과제에서는 제출 전까지 아무 말도 하지 않습니다
- 수업을 마치거나 문제집을 다 풀면 **무엇을 놓쳤는지 읽어 줍니다** — 개념마다 오늘의 결과와 그동안 쌓인 기록을 나란히 두고, 한 번뿐인 증거로는 판정하지 않습니다. 되풀이되는 착각은 학습 기록에 모이고, **그것만 겨냥한 문항을 모아 문제집 하나**로 만들 수 있습니다
- 답을 낸 뒤에는 **풀이(해설)**를 펼쳐 볼 수 있습니다. 읽어도 힌트로 세지 않습니다
- 새로고침해도 풀던 자리로 돌아오고, 뒤로 가기는 앱을 나가는 대신 한 화면 뒤로 갑니다 — 수업은 링크로 건넬 수 있습니다
- 선수개념·첫 풀이 기반 추천과 그 이유·이력, 직접 선택과 자동 추천 복귀
...
진단·첫 풀이·선행개념에 따른 규칙 기반 추천과 시간에 맞춘 새 복습 과제를 제공합니다. 수업·진단 문항·개념·용어는 DB에서 읽으며, `/authoring` 화면이나 [콘텐츠 관리 명령](docs/content-management.md)으로 새 판본을 발행합니다. 화면에 아직 없는 것은 일반 이미지·판본 비교이고, AI 호출은 없습니다. 콘텐츠는 대수 줄기를 고등학교 1학년까지 잇고 중학교의 도형·확률·통계까지 메워 **중학교 과정이 온전합니다.** 고등학교 2학년 과정, 해설 집필(문항 2,063개 중 41개뿐입니다), 전문가 검수가 그다음입니다.
