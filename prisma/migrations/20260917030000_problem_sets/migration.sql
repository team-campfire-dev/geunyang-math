...
--
-- The working tables are ordinary tables, not TEMPORARY ones: the migrator account holds CREATE and
-- DROP but not CREATE TEMPORARY TABLES, which is where the first production run of this migration
-- stopped. That run had already created the two tables below, and MySQL DDL does not roll back, so
-- this migration begins by removing what an interrupted run left behind and can be applied again.

DROP TABLE IF EXISTS `ProblemSetVersion`;
